...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{RL_flow_diagram.png}
\caption{Logical flow of reinforcement learning agent for antenna alignment. The ESP32 measures RSSI, encodes state, queries Q-table, and executes actions on stepper motors.}
\end{figure}

\subsubsection{State Space Design}
The state space represents the information available to the RL agent at each decision step. To maintain a compact representation suitable for the ESP32:
\begin{itemize}
...
\end{itemize}
The final state vector is a combination of azimuth index, tilt index, and RSSI trend index. This yields a total of $N_\theta \times N_\phi \times 3$ possible states, which is small enough to store in the ESP32’s RAM.

\subsubsection{Q-Table Size and Memory Footprint}
The Q-table size depends on the number of discrete states and actions. Let the antenna orientation be discretized into $N_\theta$ angular positions, the RSSI trend take $N_r$ discrete values, and let there be $N_a$ possible actions. Then, the total Q-table size is given by:
\begin{equation}
\left|Q\right| = N_\theta \times N_r \times N_a \dotfill
\end{equation}

For our design, $N_\theta = 36$ (10° resolution over 360°), $N_r = 3$ (increasing, stable, decreasing), and $N_a = 3$ actions, yielding:
\begin{equation}
\left|Q\right| = 36 \times 3 \times 3 = 324 \text{ Q-values} \dotfill
\end{equation}

Assuming each Q-value is stored as a 32-bit floating-point number (4 bytes), the total memory requirement is approximately 1.3 kB. This is negligible relative to the available RAM on the ESP32 (520 kB), confirming the feasibility of the embedded implementation.
...
\end{itemize}
Actions are executed via the ESP32 sending commands to the motor driver, which rotates the stepper motors in discrete steps corresponding to the state discretization. This discrete action space allows fast computation and ensures repeatable movements.
...
\subsubsection{Implementation Considerations on ESP32}

\begin{itemize}
    \item \textbf{Memory footprint:} The Q-table occupies $\approx 1.3$ KB, which is negligible relative to the ESP32 RAM (520 kB)
    ...
    \item \textbf{Real-time operation:} The ESP32 continuously measures RSSI, encodes the state, looks up the best action in the Q-table, and commands the stepper motors
